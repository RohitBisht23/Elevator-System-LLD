...
public class NeatestElevatorSelectionStrategy implements ElevatorSelectionStrategy{
    @Override
    public ElevatorController selectElevator(List<ElevatorController> controllers, int requestFloor, ElevatorDirection direction) {
        ...
        for(ElevatorController controller : controllers) {
            int nextFloorStoppage = controller.getElevatorCar().getNextStoppage();

            // Good candidate if moving same direction & not passed requested floor
            boolean isSameDirectionCandidate = controller.getElevatorCar().getMovingDirection() == direction
                    &&
                    (
